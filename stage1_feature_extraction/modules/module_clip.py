...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, return_hidden=False, video_frame=-1, return_spatial=False):
        hidden = self.visual(image.type(self.dtype), video_frame=video_frame)
        hidden = self.visual.ln_post(hidden) @ self.visual.proj
        
        # hidden shape: (batch, num_tokens, dim) where num_tokens = 1 + 49 (CLS + patches)
        
        x = hidden[:, 0, :]  # CLS token only
        
        if return_spatial:
            # Return patch tokens (exclude CLS token at index 0)
            patch_tokens = hidden[:, 1:, :]  # Shape: (batch, 49, dim)
            return patch_tokens
