...
# Path to your trained CLIP4Clip cause model checkpoint
# Train CLIP4Clip on cause captions: https://github.com/ArrowLuo/CLIP4Clip
CAUSE_MODEL_PATH = "/path/to/clip4clip_cause/pytorch_model.bin"
BATCH_SAVE_SIZE = 50
# ─────────────────────────────────────────────────────────────────────────────
...
if args.msvd:
    ...
    save_file = save_dir + '/MSVD_cause_features.pickle'
    ...
    save_file = save_dir + '/MSRVTT_cause_features.pickle'
...
model_state_dict = torch.load(CAUSE_MODEL_PATH, map_location='cpu')
cache_dir = args.cache_dir if args.cache_dir else os.path.join(str(PYTORCH_PRETRAINED_BERT_CACHE), 'distributed')
model = CLIP4Clip.from_pretrained(args.cross_model, cache_dir=cache_dir, state_dict=model_state_dict, task_config=args)
clip = model.clip.to(device)
clip.eval()

temp_dir = save_dir + '/temp_chunks_cause'
pathlib.Path(temp_dir).mkdir(parents=True, exist_ok=True)

with torch.no_grad():
    data = {}
    chunk_counter = 0
    for i in tqdm(range(len(videos)), desc="Extracting cause features"):
