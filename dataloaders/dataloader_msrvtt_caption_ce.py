...
class MSRVTT_Caption_DataLoader(Dataset):
    ...
    Params:
        ...
        features_path_cause: Path to the extracted feature file of cause.
        features_path_effect: Path to the extracted feature file.
    ...
    def __init__(
        ...
        self.feature_dict_cause = pickle.load(open(features_path_cause, 'rb'))
        self.feature_dict_effect = pickle.load(open(features_path_effect, 'rb'))
        ...
        self.feature_size_cause = self.feature_dict_cause[next(iter(self.feature_dict_cause))].shape[-1]
        self.feature_size_effect = self.feature_dict_effect[next(iter(self.feature_dict_effect))].shape[-1]
        ...
        #bbc_10k_split
        # split_dict = {"train": video_ids[:183], "val": video_ids[:183], "test": video_ids[:183]}
    ...
    def _get_video(self, choice_video_ids):
        video_mask_cause = np.zeros((len(choice_video_ids), self.max_frames), dtype=np.int64)
        video_mask_effect = np.zeros((len(choice_video_ids), self.max_frames), dtype=np.int64)

        max_video_length_cause = [0] * len(choice_video_ids)
        max_video_length_effect = [0] * len(choice_video_ids)

        video_cause = np.zeros((len(choice_video_ids), self.max_frames, self.feature_size_cause), dtype=np.float32)
        video_effect = np.zeros((len(choice_video_ids), self.max_frames, self.feature_size_effect), dtype=np.float32)
        for i, video_id in enumerate(choice_video_ids):
            video_slice_cause = self.feature_dict_cause[video_id]
            video_slice_effect = self.feature_dict_effect[video_id]

            if self.max_frames < video_slice_cause.shape[0]:
                video_slice_cause = video_slice_cause[:self.max_frames]
            if self.max_frames < video_slice_effect.shape[0]:
                video_slice_effect = video_slice_effect[:self.max_frames]

            slice_shape_cause = video_slice_cause.shape
            slice_shape_effect = video_slice_effect.shape
            max_video_length_cause[i] = max_video_length_cause[i] if max_video_length_cause[i] > slice_shape_cause[0] else slice_shape_cause[0]
            max_video_length_effect[i] = max_video_length_effect[i] if max_video_length_effect[i] > slice_shape_effect[0] else slice_shape_effect[0]
            if len(video_slice_cause) < 1:
                print("video_id: {}".format(video_id))
            else:
                video_cause[i][:slice_shape_cause[0]] = video_slice_cause

            if len(video_slice_effect) < 1:
                print("video_id: {}".format(video_id))
            else:
                video_effect[i][:slice_shape_effect[0]] = video_slice_effect

        for i, v_length in enumerate(max_video_length_cause):
            video_mask_cause[i][:v_length] = [1] * v_length

        for i, v_length in enumerate(max_video_length_effect):
            video_mask_effect[i][:v_length] = [1] * v_length

        return video_cause, video_effect, video_mask_cause, video_mask_effect, np.array([]), np.array([])
    ...
    def __getitem__(self, idx):
        ...
        video_cause, video_effect, video_mask_cause, video_mask_effect, masked_video, video_labels_index = self._get_video(choice_video_ids)
        ...
        if self.node_features == 'dgl':
            ...
            # return video_cause, video_effect, video_mask_cause, video_mask_effect, \
            #     pairs_input_caption_ids, pairs_decoder_mask, pairs_output_caption_ids
        ...
        return pairs_text, pairs_mask, pairs_segment, video_cause, video_effect, video_mask_cause, video_mask_effect, \
               pairs_masked_text, pairs_token_labels, masked_video, video_labels_index, \
               pairs_input_caption_ids, pairs_decoder_mask, pairs_output_caption_ids
    ...
    def __generate_object_features(self, fo_path, stgraph_path):
        ...
        for key in chosen_keys:
            fo_input[key] = sparse.csr_matrix(np.array(fo_file.get(key)).astype(np.float32))
