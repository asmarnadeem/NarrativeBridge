"""Train CEN on the MSVD-CTN dataset.

Typically fine-tuned from a checkpoint trained on MSRVTT-CTN. See
`scripts/train_msvd.sh` for the recommended invocation.
"""
...
def parse_args():
    p = args_mod.build_parser("Train CEN on MSVD-CTN", dataset_default="msvd")
    # MSVD default overrides — lower LR because we fine-tune from MSRVTT
    p.set_defaults(lr=5e-7, batch_size=64, batch_size_val=64,
