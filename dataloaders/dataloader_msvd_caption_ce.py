...
class MSVD_Caption_DataLoader(Dataset):
    ...
    def __init__(
        ...
        self.features_path_cause = features_path_cause
        self.features_path_effect = features_path_effect
        ...
        caption_file = os.path.join(self.data_path, "MSVD_mistral_data_causal_mapped_.pkl")
        self.feature_size_cause = self.feature_dict_cause['vid1'].shape[-1]
        self.feature_size_effect = self.feature_dict_effect['vid1'].shape[-1]
    ...
    def _get_video(self, choice_video_ids):
        ...
        video_mask_cause = np.zeros((len(choice_video_ids), self.max_frames), dtype=np.long)
        video_mask_effect = np.zeros((len(choice_video_ids), self.max_frames), dtype=np.long)
        ...
        video_cause = np.zeros((len(choice_video_ids), self.max_frames, self.feature_size_cause), dtype=np.float)
        video_effect = np.zeros((len(choice_video_ids), self.max_frames, self.feature_size_effect), dtype=np.float)
        for i, video_id in enumerate(choice_video_ids):
