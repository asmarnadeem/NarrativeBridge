#!/bin/bash
# Train CEN on MSRVTT-CTN.
#
# Single-GPU example. For multi-GPU training, increase --nproc_per_node.
...
INIT_MODEL_PATH=/path/to/univl.pretrained.bin
...
python -m torch.distributed.launch --nproc_per_node=${N_GPU} \
  ...
  --lr 5e-5 --max_words 48 --max_frames 20 \
